...
const RegVerifyemail = () => {
    ...
    useEffect(() => {
        const checkVerificationStatus = async () => {
            try {
                const response = await axios.get(`http://192.168.1.85:5001/verify-status/${email}`);
    ...
    const handleResendEmail = async () => {
        if (canResend) {
            try {
                const response = await axios.post('http://192.168.1.85:5001/resend', { email });
            ...
            Alert.alert(
                "กรุณารอ", "กรุณารอ 1 นาที ก่อนกดส่งใหม่",
            );
        }
    };
